  ...
  const [upcomingEvents, overduePayments, upcomingPayments, clientsCount, activeContracts] =
    await Promise.all([
      prisma.event.findMany({
        where: { startAt: { gte: now }, status: { not: "CANCELLED" } },
        orderBy: { startAt: "asc" },
        take: 8,
        include: { client: true },
      }),
      ...
      prisma.paymentMilestone.findMany({
        where: { status: { not: "PAID" }, dueDate: { gte: now, lte: horizon } },
        orderBy: { dueDate: "asc" },
        take: 10,
        include: { contract: { include: { client: true } } },
      }),
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <Stat label="Active contracts" value={activeContracts} href="/contracts" />
              ...
              {overduePayments.map((p) => (
                <li key={p.id} className="py-2.5 flex items-center justify-between gap-3">
                  ...
                  <div className="text-sm font-semibold text-red-700">
                    {formatMoney(p.amount, p.contract.currency)}
                  </div>
                </li>
              ))}
            </ul>
